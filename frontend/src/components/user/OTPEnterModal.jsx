      ...
      <Dialog as="div" className="relative z-10" onClose={closeModal}>
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-md sm:max-w-sm xs:max-w-[85%] transform overflow-hidden rounded-2xl bg-white p-6 xs:p-3 text-left align-middle shadow-xl transition-all">
                <Dialog.Title as="h3" className="text-lg xs:text-base font-medium leading-6 text-gray-900">
                  {getTitle()}
                </Dialog.Title>
                <div className="mt-2">
                  <p className="text-sm xs:text-xs text-gray-500">Please enter the 6-digit OTP sent to {email}.</p>
                </div>

                <form onSubmit={handleSubmit} className="mt-4">
                  <div className="flex justify-center space-x-2 xs:space-x-0.5 xs:flex-wrap xs:gap-y-2">
                    {otp.map((data, index) => (
                      <input
                        className="w-12 sm:w-11 xs:w-9 h-12 sm:h-11 xs:h-9 border-2 rounded bg-transparent outline-none text-center font-semibold text-xl sm:text-lg xs:text-base border-gray-400 focus:border-gray-700 focus:text-gray-700 text-gray-400 transition"
                        type="text"
                        name="otp"
                        maxLength="1"
                        key={index}
                  ...
                  <div className="mt-4">
                    <button
                      type="submit"
                      disabled={isVerifying || otp.join("").length !== 6}
                      className={`w-full inline-flex justify-center rounded-md border border-transparent bg-orange-800 px-4 xs:px-3 py-2 xs:py-1.5 text-sm xs:text-xs font-medium text-white ${
                        isVerifying || otp.join("").length !== 6 ? "opacity-50 cursor-not-allowed" : "hover:bg-orange-700"
                      }`}
                    >
                      {isVerifying ? "Verifying..." : "Verify OTP"}
                    </button>
                ...
                <div className="mt-4">
                  <button
                    onClick={handleResendOtp}
                    disabled={timeLeft > 0 || isSendingReset}
                    className={`w-full inline-flex justify-center rounded-md border border-transparent px-4 xs:px-3 py-2 xs:py-1.5 text-sm xs:text-xs font-medium text-white ${
                      timeLeft > 0 || isSendingReset ? "bg-gray-400 cursor-not-allowed" : "bg-orange-800/90 hover:bg-orange-700"
                    }`}
                  >
                    {timeLeft > 0 ? `Resend OTP in ${timeLeft}s` : isSendingReset ? "Sending..." : "Resend OTP"}
