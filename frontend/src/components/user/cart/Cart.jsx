        ...
        <div className="lg:w-2/3">
          <div className="bg-white rounded-2xl shadow-md p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4">
              ...
              <button
                onClick={handleClearCart}
                className="flex items-center text-red-600 hover:text-red-700 font-medium text-xs sm:text-sm transition-colors"
              >
                ...
                {cart.items.map((item) => (
                  ...
                  >
                    <CartItem
                      ...
                      stock={item.productId.colors.find(c => c.name === item.color)?.stock || 0}
                      inStock={item.inStock}
 Carre                    onRemove={() => handleRemoveItem(item.productId, item.color)}
                      onUpdateQuantity={(newQuantity) =>
                        handleUpdateQuantity(item.productId, item.color, newQuantity)
                      }
                    />
                    {isProblematic(item.productId, item.color) && (
                      <span className="absolute top-2 right-2 text-red-600 text-xs sm:text-sm font-medium">
                        {getProblematicReason(item.productId, item.color)}
                      </span>
                    )}
                  </div>
            ...
            {!isCartEmpty && (
              <button
                onClick={() => navigate("/shop")}
                className="mt-4 sm:mt-6 w-full text-orange-800 hover:text-orange-900 py-2 rounded-lg font-medium text-xs sm:text-sm transition-colors"
              >
                Continue Shopping
              </button>
            )}
