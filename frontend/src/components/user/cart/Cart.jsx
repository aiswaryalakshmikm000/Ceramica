...
//   // Use Fallback for loading, error, no user, and empty cart states
//   if (isLoading || error || !user || !cart?.items.length) {
//     return (
//       <Fallback
//         isLoading={isLoading}
//         error={error}
//         noUser={!user}
//         emptyMessage={!isLoading && !error && user && (!cart || cart.items.length === 0) ? "Your cart is empty" : null}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-2/3">
            <div className="bg-white rounded-2xl shadow-md p-6">
              ...
              {/* Show Fallback for empty cart within this section */}
              ...
              {/* Show "Continue Shopping" button only when cart has items */}
              {!isCartEmpty && (
                <button
                  onClick={() => navigate("/shop")}
                  className="mt-6 w-full text-black-800 hover:text-orange-900 py-2 rounded-lg font-medium transition-colors"
