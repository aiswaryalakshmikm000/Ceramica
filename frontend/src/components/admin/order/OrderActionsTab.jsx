              ...
              <div key={item._id} className="mt-4 bg-gray-50 p-4 rounded-md ">
                ...
                {item.returnRequest.isApproved !== null && (
                  <p className="text-sm text-gray-600 mt-1">
                    Status: {item.returnRequest.isApproved ? 'Approved' : 'Rejected'} on {formatDate(item.returnRequest.approvedAt)}
                  </p>
                )}
                {item.returnRequest.adminComment && (
                  <p className="text-sm text-gray-600 mt-1">Admin Comment: {item.returnRequest.adminComment}</p>
                )}
                {item.status === 'Return-Requested' && (
                  <div className="mt-2">
                    ...
                    <div className="flex space-x-4 mt-2">
                      <button
                        onClick={() => handleReturnItemAction(item._id, true)}
                        className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                      >
                        <CheckCircle className="h-4 w-4 mr-2" />
                        Approve
                      ...
                      <button
                        onClick={() => handleReturnItemAction(item._id, false)}
                        className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                      >
                        <XCircle className="h-4 w-4 mr-2" />
                        Reject
